...
central_matches <- elo_central_matches(central_matches)
...
```{r}
elo_actu <- function(field = c(levels(as.factor(central_matches$surface)))){
  df <- central_matches %>% filter(surface %in% field)
  df <- elo_central_matches(df)
  return(df)
  }

elo_actual <- elo_actu(c("Clay","Hard"))



>>>>>>> 8e2d57ec07fef08d740e8e719fe6973647c766d8
```
...
Run elo for different type of surface

```{r}
elo_surface <- function(surface = c(levels(as.factor(central_matches$surface)))){
...
elo_player <- function(players = IDtbl, ID) {
=======
#rrrrr
...
```{r}
<<<<<<< HEAD
elo_player <- function(ID){
  elop <- central_matches %>% select(winner_id,loser_id,elo1,elo2,tourney_date) %>% 
=======
elo_player <- function(ID,data = central_matches){
